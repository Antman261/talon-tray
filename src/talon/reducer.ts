...
type Reducer<T extends EventType = EventType> = (s: State, e: EventMap[T]) => State;

export const onEvent = <T extends EventType>(t: T, fn: Listener<T>) => getTaps(t).add(fn);
export const offEvent = <T extends EventType>(t: T, fn: Listener<T>) => getTaps(t).delete(fn);

const withWiretaps = (reduce: Reducer): Reducer => (state, event) => {
...
const reducers = {
  MIC_SELECTED: (state, { mic }) => 
    ({ ...state, mic, lastMic: mic === 'None' ? state.lastMic : mic }),
